...
impl AuthExtractor for ExpressExtractor {
    ...
    fn extract(
        ...
        collect_routes(root, bytes, path, rules, &mut model);

        model
    }
}

fn collect_top_level_units(
    root: Node<'_>,
    bytes: &[u8],
    rules: &AuthAnalysisRules,
...
fn collect_routes(
    ...
    if node.kind() == "call_expression" {
        maybe_collect_route(node, bytes, path, rules, model);
    }
    ...
    for idx in 0..node.named_child_count() {
        ...
        collect_routes(child, bytes, path, rules, model);
    }
}
...
fn maybe_collect_route(
    ...
    let Some(handler) = named_args
        .iter()
        .rev()
        .copied()
        .find(|arg| is_function_like(*arg))
    else {
        return;
    };

    let middleware_nodes: Vec<Node<'_>> = named_args[1..]
        .iter()
        .copied()
        .take_while(|arg| arg.id() != handler.id())
        .collect();
    ...
    let line = handler.start_position().row + 1;
    model.units.push(unit);
...
fn call_site_from_node(node: Node<'_>, bytes: &[u8]) -> CallSite {
    if node.kind() == "call_expression" {
        let name = node
...
fn extract_value_refs(node: Node<'_>, bytes: &[u8]) -> Vec<ValueRef> {
    match node.kind() {
...
fn member_value_ref(node: Node<'_>, bytes: &[u8]) -> Option<ValueRef> {
    ...
    let source_kind = if chain.len() >= 3 && chain[0] == "req" && chain[1] == "params" {
        ValueSourceKind::RequestParam
    } else if chain.len() >= 3 && chain[0] == "req" && chain[1] == "body" {
        ValueSourceKind::RequestBody
    } else if chain.len() >= 3 && chain[0] == "req" && chain[1] == "query" {
        ValueSourceKind::RequestQuery
    } else if chain.len() >= 3 && chain[0] == "req" && chain[1] == "session" {
        ValueSourceKind::Session
    } else if chain
        .first()
...
fn member_chain(node: Node<'_>, bytes: &[u8]) -> Vec<String> {
    if node.kind() != "member_expression" {
...
fn callee_name(node: Node<'_>, bytes: &[u8]) -> String {
    match node.kind() {
