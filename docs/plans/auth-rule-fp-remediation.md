...
| 2026-04-23 | C (foundation) | ✅ | — | auth-as-taint foundation landed: `Cap::UNAUTHORIZED_ID` bit (`src/labels/mod.rs`) + `CapName::UnauthorizedId` + `parse_cap`/`cap_to_name`/`FromStr`/`Display` plumbing; Rust phase-C rule set (`src/labels/rust.rs::phase_c_auth_rules`) declaring realtime/DB-mutation/cache sinks requiring the cap and `authz::require_{group,org,workspace,tenant,team}_member`/`check_ownership`/… as sanitizers; `phase_c_auth_rules_for_lang` dispatch in `build_lang_rules` gated by `config.scanner.enable_auth_as_taint` (default **OFF** — auth_analysis remains the authoritative path); `strip_cap_from_call_args` helper in `ssa_transfer/mod.rs` special-cases `UNAUTHORIZED_ID` sanitizers to clear the cap from argument SSA values (so `authz::require_*(…, group_id, user.id)?` actually validates `group_id`, since the call's `Result<()>` return carries no taint); `build_taint_diag` routes findings whose sink carries the `UNAUTHORIZED_ID` bit to rule id `rs.auth.missing_ownership_check.taint`; new integration test file `tests/phase_c_auth_taint_tests.rs` (3 tests — flag-off silent, unsanitized flags, `authz::require_*` suppresses) + fixture `tests/fixtures/phase_c_auth_taint/{handler_unsanitized,handler_sanitized}.rs`; 93 auth_analysis tests + full workspace green; deferred: intra-proc summary emission (`TaintTransform::ClearCapOnParams`) for cross-file authz helpers, SQL auth-gated return-path sanitizers (reuses B3 recognizer), row-ownership-equality path-sensitive integration (Phase 5.1), id-like vs non-id-like source narrowing |
