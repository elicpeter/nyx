# large_switch_go — Phase 12.6 regression fixture
...
The fixture anchors Phase 12.3's `Terminator::Switch` work: Go switches
...
per-case taint-precision work that is out of Phase 12's scope). A future
strengthening that emits real `Terminator::Switch` for Go and narrows
taint per case should bump `min_count` accordingly — the expectation is
set to the current floor so that regressions (0 findings) fail the test.
